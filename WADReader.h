...
public:
    ...
    void ReadNodesData(const uint8_t *pWADData, int offset, Node &node);                // read data for a single BSP node from the buffer at the specified offsetvoid ReadSubsectorData(const uint8_t *pWADData, int offset, Subsector &subsector);
